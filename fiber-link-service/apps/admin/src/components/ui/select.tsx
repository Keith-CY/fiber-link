import * as React from "react";
import { cn } from "../../lib/utils";

/**
 * Native select styled to match the shadcn/ui surface. Kept native (rather
 * than the Radix listbox) so URL-driven table filters stay trivial to wire
 * up and easy to drive from the Playwright acceptance harness.
 */
export const Select = React.forwardRef<HTMLSelectElement, React.SelectHTMLAttributes<HTMLSelectElement>>(
  ({ className, children, ...props }, ref) => (
    <select
      ref={ref}
      className={cn(
        "flex h-9 w-full rounded-md border border-input bg-card px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50",
        className,
      )}
      {...props}
    >
      {children}
    </select>
  ),
);
Select.displayName = "Select";
