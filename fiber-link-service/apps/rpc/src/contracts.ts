...
export const DashboardSummaryResultSchema = z.object({
  ...
  tips: z.array(
    z.object({
      ...
      amount: z.string().min(1),
      asset: z.enum(["CKB", "USDI"]),
      state: z.enum(["UNPAID", "SETTLED", "FAILED", "LIQUIDITY_PENDING", "PENDING", "PROCESSING", "RETRY_PENDING", "COMPLETED"]),
      direction: z.enum(["IN", "OUT", "WITHDRAWAL"]),
      counterpartyUserId: z.string().min(1),
      message: z.string().nullable(),
      createdAt: z.string().datetime(),
      settledAt: z.string().datetime().nullable(),
      activityType: z.enum(["TIP", "WITHDRAWAL"]).optional(),
      txHash: z.string().nullable().optional(),
      explorerUrl: z.string().nullable().optional(),
      destinationKind: z.string().nullable().optional(),
